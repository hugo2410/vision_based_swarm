...
TEST_F(SimulationTest, GridInitialization) {
    ...
    for (const auto& agent : agents) {
        EXPECT_LE(std::abs(agent.getAcceleration().first), params.maxInitialAcceleration);
        EXPECT_LE(std::abs(agent.getAcceleration().second), params.maxInitialAcceleration);
    }
}
...
TEST_F(SimulationTest, InvalidInitializationType) {
    ...
    EXPECT_TRUE(output.find("Defaulting to grid initialization") != std::string::npos);
